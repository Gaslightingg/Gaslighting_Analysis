...
def add_indicators(df: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    plus_di = 100 * plus_dm.ewm(alpha=1 / adx_period, adjust=False).mean() / out["atr"].replace(0, np.nan)
    minus_di = 100 * minus_dm.ewm(alpha=1 / adx_period, adjust=False).mean() / out["atr"].replace(0, np.nan)
    dx = 100 * (plus_di - minus_di).abs() / (plus_di + minus_di).replace(0, np.nan)
    out["adx"] = _as_series(dx, "adx").ewm(alpha=1 / adx_period, adjust=False).mean()
